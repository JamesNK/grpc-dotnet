...
Write-Host "Pipe name: $pipe_name" -ForegroundColor Cyan
Write-Host
...
foreach ($test in $allTests)
{
  ...
  dotnet run --framework $framework --use_tls $use_tls --server_host localhost --server_port $server_port --client_type httpclient --test_case $test --use_winhttp $use_winhttp --grpc_web_mode $grpc_web_mode --use_http3 $use_http3 --pipe_name $pipe_name
