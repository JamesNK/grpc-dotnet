...
namespace Grpc.Net.Client.Tests.Retry
{
    ...
    public class HedgingTests
    {
        ...
        private class WriterTestStream : Stream
        {
            ...
            {
                return WriteAsync(buffer.AsMemory(0, count), cancellationToken).AsTask();
            }

            public override async ValueTask WriteAsync(ReadOnlyMemory<byte> buffer, CancellationToken cancellationToken = default)
            {
                _currentWriteData = buffer.ToArray();
        ...
        public async Task AsyncUnaryCall_ExceedAttempts_HedgeDelay_Failure()
        {
            ...
            Console.WriteLine(callIntervals[0]);
            Console.WriteLine(callIntervals[1]);
